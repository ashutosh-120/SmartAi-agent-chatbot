...
# Ensure we don't include empty strings if settings.ALLOWED_ORIGINS is empty
_env_origins = [
    o.strip() for o in settings.ALLOWED_ORIGINS.split(",") if o.strip()
] if hasattr(settings, "ALLOWED_ORIGINS") and settings.ALLOWED_ORIGINS else []

# For development, allow everything to prevent connection issues
_final_origins = ["*"] if settings.DEBUG else (_default_origins + _env_origins)
...
app.include_router(profile_router)   # POST /profile/analyze ← multi-repo
...
async def root():
    ...
    return JSONResponse(content={
        ...
        "endpoints": {
            ...
            "profile_analyze":  "POST /profile/analyze     — Multi-repo analysis",
        }
    })
